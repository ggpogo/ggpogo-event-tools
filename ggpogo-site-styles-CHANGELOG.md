...
### 1.5.3 — bg-wave.png repointed at the recompressed upload
Eric uploaded `bg-wave-optimized.png` through the Media Library's normal "Add Media" flow, which (as expected — WordPress doesn't overwrite same-named files through that flow) gave it a new URL: `/wp-content/uploads/2026/07/bg-wave.png`, rather than replacing the original `/uploads/2026/04/bg-wave.png` in place. Fetched the new URL directly to confirm it resolves and is the expected size (**199,178 bytes**, matching the optimized PNG generated in 1.5.2) before repointing the CSS at it.

Section 24's `.gg-page-header::before` rule now points at the new URL. Benefits every `gg-page-header` page except Event Tools, which already opts out of this layer entirely as of 1.5.2. No visual change expected on any page — same image content at a smaller file size, and the effect was already visually imperceptible at 14% opacity before this change (confirmed via a live headless-browser screenshot in this session's engineering breakdown).

The original 1.86MB file at `/uploads/2026/04/bg-wave.png` is now orphaned (no longer referenced anywhere) but was left in place — deleting media library files is Eric's call, not something to do automatically.
