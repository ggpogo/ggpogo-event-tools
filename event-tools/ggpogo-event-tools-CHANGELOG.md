...
### v2.15.0 — Calendar Sync panel (new host-only feature)
**Sourcing note:** this entry was reconstructed from the in-file changelog header of the delivered v2.15.0 build (no separate delivery-notes doc existed for it), during the v2.16.0 session below when a version-numbering collision surfaced this gap.

New "Calendar Sync" tile on the host dashboard's Quick links (now a 2x2 grid) opens a panel for the GGPoGo Events Google Calendar automation, backed by a separate Cloudflare Worker (`ggpogo-calendar-sync-worker.js`):
- Sync now: triggers the Worker's on-demand sync endpoint directly from the page and shows the result inline once it returns.
- Last run / past runs: reads `calendarSync:lastRun` and `calendarSync:runs` from Firebase, so results show up from any run — manual or a future scheduled one — not just the one this page personally triggered. Surfaces added/updated/unchanged counts, per-event errors, events flagged for manual review (never auto-deleted), and cross-check notes.
- Settings: inclusion keywords, on/off toggles for the Dittobase and GO Hub cross-checks, and the default event location, all saved to `settings:calendarSync` for the Worker to read before its next run.

New Firebase paths added in this delivery (see `firebase-database-rules.json`): `settings:calendarSync` (public read, host write, matching the existing `settings:*` pattern), `calendarSync:lastRun` and `calendarSync:runs` (host-only read; write is currently open since the Worker has no Firebase Auth identity of its own — see that file's notes).

`CALENDAR_SYNC_WORKER_URL` and `CALENDAR_SYNC_KEY` (near the `CalendarSyncSettings` component) are placeholders — fill in the Worker's deployed URL and its `SYNC_TRIGGER_KEY` secret before this screen's Sync now button will work.

### v2.16.0 — Giveaway: mark a pulled winner as a No-Show
Eric wanted a way to mark a pulled giveaway winner as a no-show so they get removed and can't be pulled again, "for the same prize and/or session" — scope clarified via `AskUserQuestion` before any code was written (this project's "scope before code" convention). Two decisions came back: exclusion scope is a **per-incident host choice** ("Just this prize" vs. "Whole session"), not a fixed setting; and a no-show entrant **stays visible in the Entrants list, tagged "NO SHOW", with an undo option**, rather than disappearing.

Drawing a winner now offers a "No-Show" button next to Re-roll. Unlike Re-roll (just re-spins, the outgoing entrant stays fully eligible for everything), No-Show permanently marks them so they can't be pulled again — the host picks, per incident, whether that's just for the prize they no-showed on or for the rest of the whole session. No-shows stay visible in the Entrants list (tagged NO SHOW, or a small OUT ×N count for a prize-only mark) with an undo button in case of a mis-tap.

Data model: three new fields on `gw:entrants/{id}` — `noShow` (bool, whole-session exclusion), `noShowAt` (timestamp), `noShowPrizeIds` (object map keyed by prize id, per-prize exclusion). `entrantsObjToArray` extended to whitelist all three through (this function drops any unlisted field from local state even though Firebase persists it — the same class of gotcha that hit `onMyWay` in v2.13.8). No Firebase rules change needed: the `gw:entrants/{entrantId}` shape-validation rule only applies to an attendee's own self-entry write at creation, and host-driven `FB.update` on existing paths already covers `wonPrize`/`onMyWay`/`extraEntries` the same way — confirmed by reading the live `firebase-database-rules.json` directly during this session rather than relying on the documented precedent alone.

**Versioning note:** this feature was first delivered as "v2.15.0" against a stale project-doc snapshot of `event-tools.html` (dated 2026-07-23, `APP_VERSION v2.14.3`) that predated both v2.14.4 and the real v2.15.0 (Calendar Sync) shipping directly to this repo. The collision was caught before either delivery reached WordPress, once Eric connected this folder and the real file's `APP_VERSION` was checked directly. The feature was rebased onto the real current file and re-delivered under this non-colliding version number; see the engineering breakdown's "Session: 2026-09-05" entries for the full account.

### v2.17.0 — Giveaway: add a prize mid-session
Hosts can now add a prize while a giveaway is live, without leaving the drawing screen. An "Add prize" link next to "Drawing for" opens the same prize form used during setup (type, name, description, quantity) right there in the live session; the new prize appears in the picker immediately since `gw:prizes` is already tracked separately from `gw:session` and reactively watched by the live screen.

This formalizes something that was already technically possible but unsupported: leaving the live session screen (`onExit`) never touched `gw:session` in Firebase, so backing out to the dashboard's Setup accordion, adding a prize, and resuming already worked — it just wasn't a designed or discoverable live-session action, and invited mistakes mid-event. No data model or Firebase rules changes; this is UI-only, reusing `GiveawayApp`'s existing `addPrize` field set as a local copy inside `GiveawaySession`.
